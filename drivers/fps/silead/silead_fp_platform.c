...
#define FP_COMPATIBLE_NODE "sil,silead_fp"
...
/* HMD_1838 code for DRS-2358 by shibinbin at 2020/08/24 start */
#ifndef HQ_FACTORY_BUILD
static nav_keymap_t keymap[] = {
    { NAV_KEY_UP,       KEYCODE_SYSTEM_NAVIGATION_DOWN,   },  /* Structural design is opposite */
    { NAV_KEY_DOWN,     KEYCODE_SYSTEM_NAVIGATION_UP,   },    /* Structural design is opposite */
...
#endif
/* HMD_1838 code for DRS-2358 by shibinbin at 2020/08/24 end */
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    np = of_find_compatible_node(NULL, NULL, FP_COMPATIBLE_NODE);
    if (!np) {
        LOG_MSG_DEBUG(INFO_LOG,"of_find_compatible_node(.., '%s') failed.",FP_COMPATIBLE_NODE);
    }
    count = of_count_phandle_with_args(np, "panel", NULL);
    if (count <= 0) {
        LOG_MSG_DEBUG(INFO_LOG,"[%s]find drm_panel count(%d) fail",__func__,count);
    }
    LOG_MSG_DEBUG(INFO_LOG,"[%s]find drm_panel count(%d) ",__func__,count);
    for (i = 0; i < count; i++) {
        node = of_parse_phandle(np, "panel", i);
        LOG_MSG_DEBUG(INFO_LOG,"node%p",node);
        panel = of_drm_find_panel(node);
        LOG_MSG_DEBUG(INFO_LOG,"panel%p ",panel);
        of_node_put(node);
        if (!IS_ERR(panel)) {
            LOG_MSG_DEBUG(INFO_LOG,"[%s]find drm_panel %p successfully",__func__,panel);
            active_panel = panel;
        }
    }
    ...
    if (active_panel) {
        LOG_MSG_DEBUG(INFO_LOG,"[%s]drm_panel_notifier_register enter",__func__);
        status = drm_panel_notifier_register(active_panel,&fp_dev->notif);
        if (status)
            LOG_MSG_DEBUG(ERR_LOG,"[%s]drm_panel_notifier_register fail: %d\n",__func__,status);
    }
