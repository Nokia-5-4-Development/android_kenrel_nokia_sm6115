...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
    ...
    if ( fp_dev->avdd_ldo ) {
        err = regulator_set_load(fp_dev->avdd_ldo,22000);
        if (err)
            LOG_MSG_DEBUG(INFO_LOG, "%s: set avdd load current is fail", __func__);
        err = regulator_enable(fp_dev->avdd_ldo);/*enable regulator*/
        if (err)
            LOG_MSG_DEBUG(INFO_LOG, "%s: enable avdd is fail", __func__);
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    if (fp_dev->rst_port > 0 ) {
        ...
        if (ret < 0) {
            ...
            /* HS70 code for HS70-338 by zhuqiang at 2019/10/25 start */
            mdelay(5);
            /* HS70 code for HS70-338 by zhuqiang at 2019/10/25 end */
            gpio_direction_output(fp_dev->rst_port, 1);
        }
    }
