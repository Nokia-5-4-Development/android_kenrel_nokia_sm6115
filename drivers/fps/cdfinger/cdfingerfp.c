...
#define CF_NAV_INPUT_UP                        745
#define CF_NAV_INPUT_DOWN                    746
...
#define CF_NAV_INPUT_LONG_PRESS                744
...
static int cdfinger_parse_dts(struct device *dev, struct cdfingerfp_data *cdfinger)
{
    ...
    cdfinger->ldo_num = of_get_named_gpio(dev->of_node, "cdfinger,ldo_gpio", 0);
        if(cdfinger->ldo_num) {
            gpio_free(cdfinger->ldo_num);
            err = gpio_request(cdfinger->ldo_num,"cdfinger_ldo");
            if(err < 0) {
                CDFINGER_ERR("[%s] Failed to request GPIO=%d, err=%d",__func__,(u32)cdfinger->ldo_num, err);
            } else {
                gpio_direction_output(cdfinger->ldo_num, 1);
            }
        }
#ifdef POWER_GPIO
...
static int cdfinger_probe(struct platform_device *pdev)
{
    ...
    if(status){
        ...
        np = of_find_compatible_node(NULL, NULL, "cdfinger,fingerprint");
        if (!np) {
            CDFINGER_ERR("of_find_compatible_node(.., '%s') failed.", "cdfinger,fingerprint");
        }
        count = of_count_phandle_with_args(np, "panel", NULL);
        if (count <= 0) {
            CDFINGER_ERR("find drm_panel count(%d) fail", count);
        }
        CDFINGER_ERR("find drm_panel count(%d) ", count);
        for (i = 0; i < count; i++) {
            node = of_parse_phandle(np, "panel", i);
            CDFINGER_ERR("node%p", node);
            panel = of_drm_find_panel(node);
            CDFINGER_ERR("panel%p ", panel);

            of_node_put(node);
            if (!IS_ERR(panel)) {
                CDFINGER_ERR("find drm_panel %p successfully",panel);
                active_panel = panel;
            }
        }
#ifdef SUPPORT_ID_NUM
    ...
    cdfingerdev->notifier.notifier_call = cdfinger_fb_notifier_callback;
        ...
        if (active_panel) {
        CDFINGER_ERR("[cdfinger]drm_panel_notifier_register enter: %d\n");
        status = drm_panel_notifier_register(active_panel,&cdfingerdev->notifier);
            if (status)
            CDFINGER_ERR("[focaltech]drm_panel_notifier_register fail: %d\n", status);
        }
