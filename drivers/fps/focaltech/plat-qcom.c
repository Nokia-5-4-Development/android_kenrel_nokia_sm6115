...
struct regulator *focal_avdd;
#endif	

# undef LOG_TAG
#define LOG_TAG "sdm450"
...
int ff_ctl_init_pins(int *irq_num)
{
    ...
    focal_avdd = regulator_get(&pdev->dev, "avdd");
#endif
...
#ifdef POWER_REGULATOR
    if (on) {
        err = regulator_set_load(focal_avdd,10000);
	if(err)
	{
		FF_LOGD("load current fail\n");
		return err;
	}
	err = regulator_enable(focal_avdd);
	if(err)
	{
		FF_LOGD("enable regulator fail\n");
		return err;
	}
    }
    else {
	err = regulator_disable(focal_avdd);
	if(err)
	{
		FF_LOGD("disable regulator fail\n");
		return err;
	}
    }
#endif
