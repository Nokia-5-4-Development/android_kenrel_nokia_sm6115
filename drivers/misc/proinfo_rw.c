	...
	*/
	switch(type) {
		...
		case WT_PROINFO_remote_lock_value:
            ...
            ret = emmc_partition_rw("proinfo", 0, 2*MMC_BLOCK_SIZE, temp_buf, SIZE_4K / 8);
			printk("%s: Read bytes from proinfo! %d\n", __func__, ret);
            if (ret < 0) {
                printk("%s: Read bytes from proinfo failed! %d\n", __func__, ret);
                return -1;
            }
