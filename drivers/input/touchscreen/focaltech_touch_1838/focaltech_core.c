 ...
 * License version 2, as published by the Free Software Foundation,FTS_ESDCHECK_EN and
...
// 	if (data->lcm_lab!= NULL && data->lcm_ibb!= NULL){
// 		if (enable){
//             FTS_INFO("vsp/vsn Regulator  set to 1");
// 			if (!regulator_is_enabled (data->lcm_lab) ) {
// 				rc = regulator_enable(data->lcm_lab);
// 				if (rc) {
// 					FTS_ERROR("Regulator lcm_lab enable failed rc=%d", rc);
// 				}
// 			}
// 			else {
//                 FTS_INFO("Regulator lcm_lab is enable ----> ignore");
// 			}
// 			if (!regulator_is_enabled (data->lcm_ibb) ) {
// 				rc = regulator_enable(data->lcm_ibb);
// 				if (rc) {
// 					FTS_ERROR("Regulator lcm_ibb enable failed rc=%d", rc);
// 				}
// 			}
// 			else {
//                 FTS_INFO("Regulator lcm_ibb is enable  ----> ignore");
// 			}
// 		}
// 		else {
//             FTS_INFO("vsp/vsn Regulator  set to 0");
// 			if (regulator_is_enabled (data->lcm_lab)) {
// 				rc = regulator_disable(data->lcm_lab);
// 				if (rc)
// 				{
...
// 			}
// 			else{
// 				FTS_INFO("Regulator lcm_lab is disabled  ----> ignore");
// 			}
// 			if (regulator_is_enabled (data->lcm_ibb)) {
// 				rc = regulator_disable(data->lcm_ibb);
// 				if (rc)	{
...
// 			}
// 			else{
// 				FTS_INFO("Regulator lcm_ibb is disabled  ----> ignore");
// 			}
// 		}
// 	}
// 	else
...
/* Huaqin add for HMD by zhouzichun at 2020/07/28 start */
...
static ssize_t fts_proc_gesture_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	char buf[150] = {0};
	int rc = 0;
    int count = 0;
...
static ssize_t fts_proc_gesture_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	char cmd[256] = { 0 };
    ...
    FTS_INFO("size = %d, cmd = %s\n", (int)size, cmd);

    if (fts_data->suspended) {
        FTS_ERROR("[gesture]Already in suspend state,Please turn on the screen");
        FTS_ERROR("gesture = %d\n", fts_data->gesture_mode);
        size = -1;
    } else if (FTS_SYSFS_ECHO_ON(cmd)) {
        FTS_INFO("enable gesture");
        fts_data->gesture_mode = ENABLE;
        FTS_INFO("gesture = %d\n", fts_data->gesture_mode);
    } else if (FTS_SYSFS_ECHO_OFF(cmd)) {
        FTS_INFO("disable gesture");
        fts_data->gesture_mode = DISABLE;
        FTS_INFO("gesture = %d\n", fts_data->gesture_mode);
    }else{
        size = -1;
        FTS_ERROR( "illegal input!");
        FTS_INFO("gesture = %d\n", fts_data->gesture_mode);
    }
...
struct file_operations fts_proc_gesture = {
	.read = fts_proc_gesture_read,
	.write = fts_proc_gesture_write,
};
/*******************************************************
Description:
...
int32_t fts_extra_proc_init(void)
{
    ...
    fts_info_proc_entry = proc_create("tp_gesture", 0777, NULL, &fts_proc_gesture);
	if (NULL == fts_info_proc_entry)
	{
		FTS_ERROR( "Couldn't create proc entry!");
		return -ENOMEM;
...
#endif
/* Huaqin add for HMD by zhouzichun at 2020/07/28 end */
...
static ssize_t fts_gesture_control_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    int count = 0;
    u8 val = 0;
...
static ssize_t fts_gesture_control_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    char cmd[256] = { 0 };

    if((count - 1) > sizeof(cmd)) {
        FTS_ERROR("ERROR! input length is larger than local buffer\n");
        return -1;
    }
    memset(cmd, 0, sizeof(cmd));
    snprintf(cmd, 256, "%s", buf);
    cmd[count - 1] = '\0';
    mutex_lock(&fts_data->input_dev->mutex);
...
static DEVICE_ATTR(gesture_control, 0644, fts_gesture_control_show, fts_gesture_control_store);
...
int drm_check_dt(struct device_node *np)
{
    ...
    FTS_INFO("find drm_panel count(%d) ", count);
    for (i = 0; i < count; i++) {
        node = of_parse_phandle(np, "panel", i);
        FTS_INFO("node:%p", node);
        panel = of_drm_find_panel(node);
        FTS_INFO("panel:%p ", panel);
                                 ...
                                 unsigned long event, void *data)
{
    ...
    case DRM_PANEL_BLANK_UNBLANK:
        if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
            ...
            FTS_INFO("resume: event = %lu, TP_RESUME\n", event);
            queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
        }
        break;
    case DRM_PANEL_BLANK_POWERDOWN:
        if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
            FTS_INFO("suspend: event = %lu,TP_SUSPEND\n", event);
            cancel_work_sync(&fts_data->resume_work);
            fts_ts_suspend(ts_data->dev);
        } else if (DRM_PANEL_EVENT_BLANK == event) {
            FTS_INFO("suspend: event = %lu, not care\n", event);
...
#endif
/* HS70 add for HS70-1007 by gaozhengwei at 2019/11/22 end */
//HMD modify for SR-ZQL1838-01-14 by zhouzichun at 2020/05/17 start
#if defined(ILI_TOUCH)
    #if defined(CONFIG_TOUCHSCREEN_ILI_7807G)
    extern bool is_ilitek_tp;
    #endif
    #if defined(CONFIG_TOUCHSCREEN_ILI_7806S)
    extern bool is_ilitek_tp_7806s;
    #endif
#endif
//HMD modify for SR-ZQL1838-01-14 by zhouzichun at 2020/05/17 end
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->dev->of_node) {
        ...
        if (ret) {
            ...
            if(!check_default_tp(ts_data->dev->of_node,"focaltech,fts"))
            {
                err=-EPROBE_DEFER;
                FTS_ERROR("EPROBE_DEFER");
            }else{
                FTS_ERROR("ENODEV");
                err= -ENODEV;
...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    FTS_INFO("Driver Data:%s",Driver_Data);
    //HMD modify for SR-ZQL1838-01-14 by zhouzichun at 2020/05/17 start
#if defined(ILI_TOUCH)
    #if defined(CONFIG_TOUCHSCREEN_ILI_7807G)
    if(is_ilitek_tp) {
        FTS_INFO("ILITEK 7807G tp has probe ok, don't run FTS probe\n");
        return -ENOMEM;
    }
    #endif
    #if defined(CONFIG_TOUCHSCREEN_ILI_7806S)
    if(is_ilitek_tp_7806s) {
        FTS_INFO("ILITEK 7806S tp has probe ok, don't run FTS probe\n");
        return -ENOMEM;
    }
    #endif
#endif
