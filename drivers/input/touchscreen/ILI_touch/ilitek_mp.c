...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
    ...
    tmp_line++;
    tmp_len += snprintf(csv + tmp_len, (file_size - tmp_len), "Firmware Mp Version ,0x%x\n", core_mp.fw_mp_ver);
    tmp_line++;
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	int i, k, ret = 0, len = 0, rd_frame_num = 1;
	u8 cmd[15] = {0};
	u8 *ori = NULL;
	...
	for (k = 0; k < rd_frame_num; k++) {
		...
		for (i = 0; i < core_mp.frame_len; i++) {
			...
			if ((tmp & 0x8000) == 0x8000)
				frame0_buf[k][i] = tmp - 65536;
			else
				frame0_buf[k][i] = tmp;

			// multiple case frame3 = frame1 - frame2
			if (tItems[index].bench_mark_multiple && rd_frame_num == k + 1)
				frame0_buf[k+1][i] = frame0_buf[k-1][i] - frame0_buf[k][i];
		}
	}
...
// HMD modify for SR-ZQL1838-01-14 by zhouzichun at 2020/05/11 start
static int open_test_sp(int index)
{
	struct mp_test_P540_open * open = (struct mp_test_P540_open *) kcalloc(tItems[index].frame_count, sizeof(struct mp_test_P540_open), GFP_KERNEL);
	int i = 0, x = 0, y = 0, ret = 0, addr = 0;
	int Charge_AA = 0, Charge_Border = 0, Charge_Notch = 0, full_open_rate = 0;
	char str[512] = {0};
	if (ERR_ALLOC_MEM(open)) {
		ipio_err("Failed to allocate struct mp_test_P540_open\n");
		return -EMP_NOMEM;
	}
	ipio_debug("index = %d, desp = %s, Frame Count = %d\n",
		index, tItems[index].desp, tItems[index].frame_count);
...
{
	struct mp_test_open_c * open = (struct mp_test_open_c *) kcalloc(tItems[index].frame_count, sizeof(struct mp_test_open_c), GFP_KERNEL);
...
// HMD modify for SR-ZQL1838-01-14 by zhouzichun at 2020/05/11 end
...
static int mp_show_result(bool lcm_on)
{
	...
	if (pass_item_count == 0) {
		...
		ret = MP_DATA_FAIL;
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", CSV_LCM_ON_PATH,idev->md_name, get_date_time_str(), ret_fail_name);
		else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", CSV_LCM_OFF_PATH,idev->md_name, get_date_time_str(), ret_fail_name);
	} else {
		...
		if (lcm_on)
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", CSV_LCM_ON_PATH,idev->md_name, get_date_time_str(), ret_pass_name);
		else
			snprintf(csv_name, (CSV_FILE_SIZE - csv_len), "%s/%s_%s_%s.csv", CSV_LCM_OFF_PATH,idev->md_name, get_date_time_str(), ret_pass_name);
	}
...
static void ilitek_tddi_mp_init_item(void)
{
    ...
    core_mp.fw_mp_ver = idev->chip->fw_mp_ver;
