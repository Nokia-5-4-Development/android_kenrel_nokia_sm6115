...
int ilitek_tddi_ic_watch_dog_ctrl(bool write, bool enable)
{
	...
	while (timeout > 0) {
		...
		ipio_debug("ret = %x\n", ret);
		if(ret == 0x83) {
			ipio_err("ic is not in ICE mode,return\n");
			return -1;
		}
		if (enable) {
			if (ret == TDDI_WDT_ON)
...
int ilitek_tddi_ic_get_panel_info(void)
{
	...
	if (idev->info_from_hex && (idev->chip->core_ver >= CORE_VER_1410)) {
		...
		ipio_info("panel_wid = %d   panel_hei=%d\n", idev->panel_wid,idev->panel_hei);
		goto out;
	}
...
int ilitek_tddi_ic_dummy_check(void)
{
	...
	if (ilitek_ice_mode_write(WDT9_DUMMY2, wdata, sizeof(u32)) < 0)
		ipio_err("Write dummy error\n");

	if (ilitek_ice_mode_read(WDT9_DUMMY2, &rdata, sizeof(u32)) < 0)
		ipio_err("Read dummy error\n");

	if (rdata != wdata){
		ipio_err("Dummy check incorrect, rdata = %x wdata = %x \n", rdata, wdata);
		return -1;
	}
