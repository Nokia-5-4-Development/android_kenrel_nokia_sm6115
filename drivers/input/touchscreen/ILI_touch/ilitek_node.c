...
static ssize_t ilitek_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (idev->info_from_hex) {
		ch[1] = idev->fw_info[48];
		ch[2] = idev->fw_info[49];
		ch[3] = idev->fw_info[50];
		ch[4] = idev->fw_info[51];
		snprintf(buf, 150, "IC=ILI7807G module=%s fw_ver=%d.%d.%d.%d \n",idev->md_name,ch[1], ch[2], ch[3], ch[4]);
	}
	else
	{
...
static ssize_t ili_proc_gesture_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
    ...
    if (idev->tp_suspend) {
        ipio_err("[gesture]Already in suspend state,Please turn on the screen");
        ipio_err("gesture = %d\n", idev->gesture);
        size = -1;
    }else if (cmd[0] == '1') {
        ipio_info("enable gesture");
        idev->gesture = ENABLE;
		ipio_info("gesture = %d\n", idev->gesture);
    }else if (cmd[0] == '0') {
        ipio_info("disable gesture");
        idev->gesture  = DISABLE;
		ipio_info("gesture = %d\n", idev->gesture);
    }else{
		size = -1;
        ipio_err( "illegal input!");
		ipio_info("gesture = %d\n", idev->gesture);
    }
...
struct file_operations ili_proc_gesture = {
...
static ssize_t zh_ili_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	if (g_user_buf[1] == EMP_MODE) {
		ipio_err("Failed to switch MP mode, abort!\n");
	} else if (g_user_buf[1] == EMP_FW_PROC) {
		ipio_err("FW still upgrading, abort!\n");
	} else if (g_user_buf[1] == EMP_FORMUL_NULL) {
		ipio_err("MP formula is null, abort!\n");
	} else if (g_user_buf[1] == EMP_INI) {
		ipio_err("Not found ini file, abort!\n");
	} else if (g_user_buf[1] == EMP_NOMEM) {
		ipio_err("Failed to allocated memory, abort!\n");
	} else if (g_user_buf[1] == EMP_PROTOCOL) {
		ipio_err("Protocol version isn't matched, abort!\n");
	} else if (g_user_buf[1] == EMP_TIMING_INFO) {
		ipio_err("Failed to get timing info, abort!\n");
	}
...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] 20200707-crc issue\n");
