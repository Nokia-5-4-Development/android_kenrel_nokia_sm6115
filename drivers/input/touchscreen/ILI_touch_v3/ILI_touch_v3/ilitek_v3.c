...
int ili_sleep_handler(int mode)
{
	...
	case TP_RESUME:
		...
		if (ilits->gesture) {
		    ...
		    #if ILI_POWER_SOURCE_CUST_EN
			ili_lcm_power_source_ctrl(ilits, 1);//disable vsp/vsn
...
static int ilitek_get_tp_module(void)
{
	...
	if (NULL == chosen)
		...
		of_property_read_string(chosen, "bootargs", &ilits->panel_name);
		ILI_INFO("cmdline:%s\n", ilits->panel_name);
	}
    if (NULL != strstr(ilits->panel_name, "mdss_dsi_ili7806s_txd_boe_sb_hd_video")) {
		ILI_INFO("the specified display module is ili7806s_txd_boe_sb\n");
		return MODEL_BOE_SB;
	}
	else {
		ILI_ERR("can't find the specified display module  default to ili7806s_txd_boe_sb\n");
		return MODEL_BOE_SB;
	}
...
static void ili_update_tp_module_info(void)
{
	...
	case MODEL_BOE:
		...
		break;
    /* HMD code for DRSTRANGE-234 by zhouzichun at 2020/08/29 start */
	case MODEL_BOE_SB:
		ilits->md_name = "ILI7806S_txd_boe_sb";
		ilits->md_fw_filp_path = BOE_SB_FW_FILP_PATH;
		ilits->md_fw_rq_path = BOE_SB_FW_REQUEST_PATH;
		ilits->md_ini_path = BOE_SB_INI_NAME_PATH;
		ilits->md_ini_rq_path = BOE_SB_INI_REQUEST_PATH;
		ilits->md_fw_ili = CTPM_FW_BOE_SB;
		ilits->md_fw_ili_size = sizeof(CTPM_FW_BOE_SB);
		break;
    ...
    /* hmd add node for sensor by zhouzichun at 2020/09/07 start */
	#ifdef CONFIG_HW_INFO
		set_tp_module_name(ilits->md_name);
	#endif
	/* hmd add node for sensor by zhouzichun at 2020/09/07 end */
...
void ili_dev_remove(void)
{
    ...
    ili_v3_test_node_exit(&hwinfo_device_v3);
