...
#define DTS_OF_NAME	"tchip,ili7806s"
...
// static int ili_v3_check_default_tp(struct device_node *dt, const char *prop)
// {
...
static void ilitek_plat_charger_init(void)
{
	int ret = 0;
...
{
    #if defined(CONFIG_DRM_PANEL) || defined(ILI_POWER_SOURCE_CUST_EN)
    int ret;
    #endif
	ILI_INFO("platform probe\n");
    /* HMD code for HS70-133 by zhouzichun at 2020/08/28 start */
    ILI_INFO( "Driver Data:%s",Driver_Data);
    is_ilitek_tp_7806s = false;
    #if defined(CONFIG_DRM_PANEL)
    ...
    #endif
    /* HMD code for HS70-133 by zhouzichun at 2020/08/28 end */
#if REGULATOR_POWER
...
/* HS70 add for SR-ZQL1871-01-177 by gaozhengwei at 2019/11/02 start */
	...
	atomic_set(&(ilits->lcm_lab_power), 0);
	atomic_set(&(ilits->lcm_ibb_power), 0);
	ret = ili_lcm_bias_power_init(ilits);

	if (ret) {
		ILI_ERR("power resource init error!\n");
...
/* HS70 add for SR-ZQL1871-01-177 by gaozhengwei at 2019/11/02 end */
...
#endif
    /* HMD code for HS70-133 by zhouzichun at 2020/08/28 start */
    is_ilitek_tp_7806s = true;
    ILI_INFO("set is_ilitek_tp_7806s = true\n");
    /* HMD code for HS70-133 by zhouzichun at 2020/08/28 end */
	...
	return 0;
ili_probe_failed:
    ILI_ERR("ili_probe_failed:\n");
	ili_irq_unregister();
    gpio_free(ilits->tp_int);
	gpio_free(ilits->tp_rst);
#if ILI_POWER_SOURCE_CUST_EN
	ili_lcm_power_source_ctrl(ilits, 0);
	ili_lcm_bias_power_deinit(ilits);
err_power_resource_init_fail:
#endif
ili_probe_failed_spi:
    ILI_ERR("ili_probe_failed_spi:\n");
	...
	ipio_kfree((void **)&ilits->hwif->info);
	return -ENODEV;
...
static int ilitek_plat_remove(void)
{
	...
	ili_dev_remove();
#if defined(CONFIG_DRM)
	ILI_INFO("remove drm notifier\n");
	#if defined(CONFIG_DRM_PANEL)
	ILI_INFO("drm_panel_notifier_unregister\n");
	if (active_panel)
		drm_panel_notifier_unregister(active_panel, &ilits->notifier_fb);
	#endif
	#if defined(CONFIG_DRM_MSM)
	if (msm_drm_unregister_client(&idev->notifier_fb))
		ILI_ERR("[DRM]Error occurred while unregistering fb_notifier.\n");
	#endif
#endif//CONFIG_DRM
	return 0;
}
...
static int __init ilitek_plat_dev_init(void)
{
    ...
    #if defined(CONFIG_TOUCHSCREEN_ILI_7807G)
    if(is_ilitek_tp) {
        ILI_INFO("ILITEK 7807G tp has probe ok, don't run 7806S probe\n");
