...
struct platform_device hwinfo_device_v3= {
	.name = HWINFO_NAME,
	.id = -1,
};
...
static ssize_t ilitek_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	if (ilits->info_from_hex) {
		...
		snprintf(buf, 150, "IC=ILI7806S module=%s fw_ver=%d.%d.%d.%d \n",ilits->md_name,ch[1], ch[2], ch[3], ch[4]);
	}
	else
	{
		snprintf(buf, 150, "IC=ILI7806S module=%s fw_ver=NA\n",ilits->md_name);
	}
	ILI_INFO( "Driver Data:%s",Driver_Data);
...
static ssize_t ili_proc_gesture_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	char buf[150] = {0};
	int rc = 0;
    int count = 0;
    ...
    count = snprintf(buf, PAGE_SIZE, "Gesture Mode:%s\n",
                     ilits->gesture ? "On" : "Off");
    mutex_unlock(&ilits->touch_mutex);
	rc = simple_read_from_buffer(buff, size, pPos, buf, strlen(buf));
	return rc;
}
static ssize_t ili_proc_gesture_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	char cmd[256] = { 0 };
    ...
    if (ilits->tp_suspend) {
        ILI_ERR("[gesture]Already in suspend state,Please turn on the screen");
        ILI_ERR("gesture = %d\n", ilits->gesture);
        size = -1;
    }else if (cmd[0] == '1') {
...
static struct file_operations ili_proc_gesture = {
	.read = ili_proc_gesture_read,
	.write = ili_proc_gesture_write,
};
...
*******************************************************/
static int dev_mkdir(char *name, umode_t mode);
static int32_t ilitek_extra_proc_init(void)
{
	...
	{
		ILI_INFO( "Create proc entry success!");
	}
	ilitek_info_proc_entry = proc_create("tp_gesture", 0777, NULL, &ili_proc_gesture);
	if (NULL == ilitek_info_proc_entry)
	{
		ILI_ERR( "Couldn't create proc entry!");
		return -ENOMEM;
...
static ssize_t ili_gesture_control_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    int count = 0;
...
static ssize_t ili_gesture_control_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    char cmd[256] = { 0 };
...
static DEVICE_ATTR(gesture_control, 0644, ili_gesture_control_show, ili_gesture_control_store);
...
static int ili_test_node_init(struct platform_device *tpinfo_device)
{
    int err=0;
    err = sysfs_create_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
    if (0 != err)
    {
        ILI_ERR( "ERROR: ITO node create failed.");
        sysfs_remove_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
        return -EIO;
    }
    else
    {
        ILI_INFO("ITO node create_succeeded.");
    }
    return err;
}

int ili_v3_test_node_exit(struct platform_device *tpinfo_device)
{
	sysfs_remove_group(&tpinfo_device->dev.kobj, &ili_test_attribute_group);
	return 0;
}
...
void ili_node_init(void)
{
	...
	netlink_init();
    /* HMD code for DRDOOM-27 by zhouzichun at 2020/09/01 start */
    platform_device_register(&hwinfo_device_v3);
    ili_test_node_init(&hwinfo_device_v3);
    ilitek_extra_proc_init();//creat /proc/tp_info
    /* HMD code for DRDOOM-27 by zhouzichun at 2020/09/01 end */
}
